...
<bs:page>
    ...
    <script type=text/javascript>
		...
		function populateSlackNotificationDialog(id){
			...
			jQuerySlacknotification.each(ProjectBuilds.projectSlacknotificationConfig.slackNotificationList, function(thing, config){
				if (id === config[0]){
				    ...
				    jQuerySlacknotification('#slackNotificationFormContents input.payloadFormat').each(function(i){
						if(this.value === slacknotification.payloadFormat){
							this.checked = true;
						} else {
							this.checked = false;
						}
		...
		function selectCorrectRadio(id){
			jQuerySlacknotification('#slackNotificationFormContents input.payloadFormat').each(function(i){
				if(this.value == jQuerySlacknotification('#payloadFormat_'+id).val()){
					this.checked = true;
				} else {
					this.checked = false;
				}
